...
@Insert("insert into daily_operation(id ,car_id,cost,type,ocurrence_time,remark,creator,create_time) values (#{id},#{car_id},#{cost},#{type},#{ocurrence_time},#{remark},#{creator},#{create_time})")
    public void addDailyOperation (DailyOperation dailyOperation);

@Update("update daily_operation set car_id = #{car_id},cost = #{cost},type = #{type},ocurrence_time = #{ocurrence_time},remark = #{remark},creator = #{creator},create_time = #{create_time} where id = #{id}")
